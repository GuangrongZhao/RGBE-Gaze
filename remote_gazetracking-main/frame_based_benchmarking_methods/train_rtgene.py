...
parser.add_argument("--epoch", default=120, type=int, help="The number of epochs")
...
if __name__ == "__main__":
    ...
    for ii in range(train_start, train_end):
        for jj in [1, 2, 3, 4, 5, 6]:
            ...
            total_rows += f_data['data'].shape[1]  # Assuming 'data' dataset has consistent size along axis 1

    train_data_left = np.empty((total_rows, 1, 36, 60))
    train_label = np.empty((total_rows, 2))
    train_headpose = np.empty((total_rows, 2))
    train_data_right = np.empty((total_rows, 1, 36, 60))
    ...
    for ii in range(train_start, train_end):
        print(ii)
        for jj in [1, 2, 3, 4, 5, 6]:
            f_data_left = h5py.File(os.path.join(args.data_path, f'two_branch_left_eye_normalized_user_{ii}_exp{jj}.h5'), 'r')
            f_data_right = h5py.File(os.path.join(args.data_path, f'two_branch_right_eye_normalized_user_{ii}_exp{jj}.h5'), 'r')
            f_label_head_pose = h5py.File(os.path.join(args.data_path, f'face_norm_headpose_user_{ii}_exp{jj}.h5'), 'r')
    ...
    trainDataset = Data.TensorDataset(
        (torch.from_numpy(train_data_left).type(torch.FloatTensor) / 255),
        (torch.from_numpy(train_data_right).type(torch.FloatTensor) / 255),
    ...
    testDataset = Data.TensorDataset(
        (torch.from_numpy(test_data_left).type(torch.FloatTensor) / 255),
    ...
    with open(os.path.join(args.model_save_path, "rtgene"), 'w') as outfile:
        for epoch in tqdm(range(args.epoch)):
            ...
            if epoch % 10 == 0:
                torch.save(model.state_dict(), os.path.join(args.model_save_path, f"rtgene_Iter_{epoch}.pt"))
