...
if __name__ == "__main__":
    ...
    for ii in range(train_start, train_end):
        ...
        for jj in [1, 2, 3, 4, 5, 6]:
            image_path_list.append(os.path.join(
                args.data_path, f"448_448_face_normalized_user_{ii}_exp{jj}.h5"))
            label_path_list.append(os.path.join(
                args.data_path, f"combine_pitch_yaw_user_{ii}_exp{jj}.h5"))
    ...
    with open(os.path.join(args.model_save_path, "fullface"), 'w') as outfile:
        for epoch in tqdm(range(args.epoch)):
            ...
            if epoch % 10 == 0:
                torch.save(model.state_dict(), os.path.join(args.model_save_path, f"fullface_Iter_{epoch}.pt"))
