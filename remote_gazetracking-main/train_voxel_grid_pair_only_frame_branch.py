...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", default="G:/remote_apperance_gaze_dataset/processed_data/data_network_training_for_event_method_eva/", help="Path to the dataset")
    parser.add_argument("--model_read_path", default="G:/remote_apperance_gaze_dataset/processed_data/pre_trained_models/checkpoints", help="Path to read the model checkpoints")
    parser.add_argument("--result_save_path", default="G:/remote_apperance_gaze_dataset/processed_data/pre_trained_models_for_event_method_eva/checkpoints", help="Path to save the model checkpoints")
    ...
    for ii in range(train_start, train_end):
        for jj in [1, 2, 3, 4, 5, 6]:
            f_data = h5py.File(os.path.join(args.data_path, f'64_96_left_eye_normalized_user_{ii}_exp{jj}.h5'), 'r')
            f_label = h5py.File(os.path.join(args.data_path, f'L_pitch_yaw_user_{ii}_exp{jj}.h5'), 'r')
            f_head = h5py.File(os.path.join(args.data_path, f'headpose_L_pitch_yaw_user_{ii}_exp{jj}.h5'), 'r')
    ...
    with open(os.path.join(args.result_save_path, "only_frame_dilated_L_eye"), 'w') as outfile:
        for epoch in tqdm(range(args.epoch)):
            ...
            if epoch % 10 == 0:
                torch.save(model.state_dict(), os.path.join(args.result_save_path, f"only_frame_dilated_L_eye_Iter_{epoch}.pt"))
