...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_read_path", default="G:/remote_apperance_gaze_dataset/processed_data/pre_trained_models_for_event_method_eva/checkpoints/", help="Path to read the model checkpoints")
    parser.add_argument("--result_save_path", default="G:/remote_apperance_gaze_dataset/processed_data/pre_trained_models_for_event_method_eva/ui_frame_branch/", help="Path to save the results")
    ...
    if args.network == "DilatedNet":
        ...
        state_dict = torch.load(
            os.path.join(args.model_read_path, 'only_frame_dilated_R_eye.pt'),
    ...
    for user in userlist:
        ...
        for ii in range(user, user+1):
            for jj in [1, 2, 3, 4, 5, 6]:
                f_data = h5py.File(os.path.join(
                    args.data_path, '64_96_right_eye_normalized_user_' + str(ii) + '_exp' + str(jj) + '.h5'), 'r')
                f_label = h5py.File(os.path.join(
                    args.data_path, 'R_pitch_yaw_user_' + str(ii) + '_exp' + str(jj) + '.h5'), 'r')
                f_head = h5py.File(os.path.join(
                    args.data_path, 'headpose_R_pitch_yaw_user_' + str(ii) + '_exp' + str(jj) + '.h5'), 'r')
        ...
        with open(os.path.join(args.result_save_path, 'user_' + str(user) + "R"), 'w') as outfile:
            with torch.no_grad():
                model.eval()
                Testloss = 0
                Testlosstotal = 0
