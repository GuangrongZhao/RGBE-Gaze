...
**convert2eventspace** offers the RGB Images convert from the raw RGB Images using the homography matrix T_{h} i.e., 'matlab_processed/tform_total.mat', the homography matrix T_{h} is caculated using the code 'matlab_processed/twoimage_homography.m', and the code transform the 66 participant's raw RGB Images to align the event stream's space is presented in  'matlab_processed/transfer_image_homography.m'. In addition, the file 'timestamp.txt' records internal timestamp provided by FLIR BFS-U3-16S2C, and 'timestamp_win.txt' records the windows system time provided by function 'time.time()' in Python and 'timestamp_cpu.txt' records cpu timestamp provided by function 'cv2.getTickCount' of opencv.
