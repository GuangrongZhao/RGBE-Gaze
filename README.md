...
The **RGBE_Gaze_dataset** folder consists of the data generated by Prophesee EVK4 (Event Streams), FLIR BFS-U3-16S2C (RGB Images), Gazepoint GP3 HD (Gaze References) and mouse click positions on the screen (Sparse PoG ground truth)

This directory contains 66 subdirectories corresponding to 66 participants.  The six sessions' data is contained in six separate directory i.e., exp1-exp6. Within each session directory, you will find a 'convert2eventspace' folder housing RGB Images, a 'prophesee' folder containing the event stream, a 'gazepoint' folder with Gaze References, and a 'click_gt_cpu.txt' text file documenting mouse click positions alongside corresponding windows CPU timestamps.


`/EV_Eye_dataset/raw_data/Data_davis/user1/left/session_1_0_2/user_1.csv`. The `creation_time.txt` file records the system time of the computer when DAVIS346 started collecting.
