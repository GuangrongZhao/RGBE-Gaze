...
if __name__ == "__main__":
    ...
    parser.add_argument("--network", default="GazeNet", help="The type of network")
    ...
    for ii in range(train_start,train_end):
        for jj in [1,2,3,4,5,6]:
            ...
            f_data = h5py.File(os.path.join(
                'G:/remote_apperance_gaze_dataset/processed_data/data_for_frame_gaze_network_training/R_eye_normalized_user_' + str(
            ...
            train_data_temp = ((f_data['data'][()]).T).reshape(-1,1,36,60)
...
####################################### #########################################################################################################
    ...
    for ii in range(test_start,test_end):
         for jj in [1,2,3,4,5,6]:
                ...
                f_data = h5py.File(os.path.join('G:/remote_apperance_gaze_dataset/processed_data/data_for_frame_gaze_network_training/R_eye_normalized_user_'+str(ii)+'_exp'+str(jj)+'.h5'),'r')   #创建一个h5文件，文件指针是f
                f_label = h5py.File(os.path.join('G:/remote_apperance_gaze_dataset/processed_data/data_for_frame_gaze_network_training/R_pitch_yaw_user_'+str(ii)+'_exp'+str(jj)+'.h5'),'r')   #创建一个h5文件，文件指针是f
                f_head = h5py.File(os.path.join('G:/remote_apperance_gaze_dataset/processed_data/data_for_frame_gaze_network_training/R_headpose_user_'+str(ii)+'_exp'+str(jj)+'.h5'),'r')   #创建一个h5文件，文件指针是f
...
#################################### Configure data loader#################################################
    ...
    if args.network == "GazeNet":
        model =  GazeNet.Net(1).to(device)
    ...
    with open(os.path.join("G:/remote_apperance_gaze_dataset/processed_data/pre_trained_models/", f"gazenet_R_eye"), 'w') as outfile:
        for epoch in tqdm(range(args.epoch)):
            ...
            if epoch % 10== 0:
               torch.save(model.state_dict(), os.path.join('G:/remote_apperance_gaze_dataset/processed_data/pre_trained_models/checkpoints/', f"gazenet_R_eye_Iter_{epoch}.pt"))
