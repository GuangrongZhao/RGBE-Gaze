...
if __name__ == "__main__":
    ...
    for ii in range(train_start, train_end):
        for jj in [1, 2, 3, 4, 5, 6]:
            f_data = h5py.File(os.path.join(args.data_path,
                                            '36_60_R_eye_normalized_user_' + str(ii) + '_exp' + str(jj) + '.h5'), 'r')
            f_label = h5py.File(os.path.join(args.data_path,
                                             'R_pitch_yaw_user_' + str(ii) + '_exp' + str(jj) + '.h5'), 'r')
            f_head = h5py.File(os.path.join(args.data_path,
                                            'R_headpose_user_' + str(ii) + '_exp' + str(jj) + '.h5'), 'r')
    ...
    with open(os.path.join(args.model_save_path, f"gazenet_R_eye"), 'w') as outfile:
            ...
            if epoch % 10 == 0:
                torch.save(model.state_dict(),
                           os.path.join(args.model_save_path,
                                        f"gazenet_R_eye_Iter_{epoch}.pt"))
