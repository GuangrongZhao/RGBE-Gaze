...
for i = 1:66
    ...
    for j =1:6
        ...
        left_gazenorm = load (fullfile(['D:\remote_dataset\processed_data_old\user_',num2str(i),'\exp',num2str(j), '\convert_frame_normalized\left_eye\left_rv_gazenorm_list.mat'])).left_rv_gazenorm_list;
        right_gazenorm = load (fullfile(['D:\remote_dataset\processed_data_old\user_',num2str(i),'\exp',num2str(j)', '\convert_frame_normalized\right_eye\right_rv_gazenorm_list.mat'])).right_rv_gazenorm_list;
...
%         ['D:\remote_dataset\user_',num2str(i),'\exp',num2str(j)]
        [no_detected_faces_index] = textread(fullfile(['D:\remote_dataset\processed_data_old\user_',num2str(i),'\exp',num2str(j),'\convert_frame_normalized\','no_detected_faces_index.txt']), '%f');
        no_detected_faces_index = no_detected_faces_index+1; % python 相对matlab查找到的索引要加1，图片从0开始
        
        
        if j == 5||j==6 % smooth 采集只有前5000 有效
            names_file(5001:end) = [];
            time_frame(5001:end) = [];
            no_detected_faces_index_drop = find(no_detected_faces_index>5000);
            no_detected_faces_index(no_detected_faces_index_drop) = [];
...
% parfor k = indexArray 
            k =  indexArray (sk);
% % k
%             left_eye_normalized = (imread(cell2mat(fullfile(['D:\remote_dataset\processed_data_old\user_',num2str(i),'\exp',num2str(j),'\convert_frame_normalized','\left_eye\', names_file(k)]))));
%             right_eye_normalized = (imread(cell2mat(fullfile(['D:\remote_dataset\processed_data_old\user_',num2str(i),'\exp',num2str(j),'\convert_frame_normalized','\right_eye\', names_file(k)]))));
%             face_normalized = (imread(cell2mat(fullfile(path_folder,'\convert2eventspace\', names_file(k)))));
            face_normalized = (imread(cell2mat(fullfile(['D:\remote_dataset\processed_data_old\user_',num2str(i),'\exp',num2str(j),'\convert_frame_normalized','\face\',  names_file(k)]))));
            
            names_file_list(sk) =  names_file(k);
...
%             face_normalized = histeq( face_normalized);
...
%             T_right_gazenorm = rotation_vector2matrix(right_gazenorm(k,:));
%             
...
%         headpose_right_pitch_yaw_list(del_index , :, :) = [];
        hdf5write (['C:\Users\Aiot_Server\Desktop\remote_dataset\data_for_gaze_network\','histeq_448_448_face_normalized_user_',num2str(i),'_exp',num2str(j),'.h5'],'/data', face_normalized_list);
        
        
        save(['D:/remote_dataset/processed_data_voxel_grid/user_',num2str(i),'/exp',num2str(j),'/image_list.mat'], 'names_file_list');
      
        
%         hdf5write (['C:\Users\Aiot_Server\Desktop\remote_dataset\data_for_gaze_network\','timedifference_down_36_60_left_eye_normalized_user_',num2str(i),'_exp',num2str(j),'.h5'],'/data', left_eye_normalized_list);
%         hdf5write (['C:\Users\Aiot_Server\Desktop\remote_dataset\data_for_gaze_network\','timedifference_down_36_60_right_eye_normalized_user_',num2str(i),'_exp',num2str(j),'.h5'],'/data', right_eye_normalized_list);
...
%         hdf5write (['D:\remote_dataset\data_for_gaze_network\','R_headpose_user_',num2str(i),'_exp',num2str(j),'.h5'],'/data', headpose_right_pitch_yaw_list);
...
%         hdf5write (['C:\Users\Aiot_Server\Desktop\remote_dataset\data_for_gaze_network\','timedifference_R_pitch_yaw_user_',num2str(i),'_exp',num2str(j),'.h5'],'/data', R_pitch_yaw_angle_list);
