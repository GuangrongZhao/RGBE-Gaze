...
if __name__ == "__main__":
    ...
    if args.network == "DilatedNet":
        ...
        state_dict = torch.load(
            'G:/remote_apperance_gaze_dataset/processed_data/pre_trained_models_for_frame_method_eva/checkpoints/dilated_R_eye_Iter_10.pt', map_location=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
    print(model)
    ...
    for user in userlist:
        ...
        for ii in range(user,user+1):
            for jj in [1, 2, 3, 4, 5, 6]:
                ...
                f_data = h5py.File(os.path.join(
                    'G:/remote_apperance_gaze_dataset/processed_data/data_network_training_for_frame_method_eva/64_96_R_eye_normalized_user_' + str(
        ...
        with open(os.path.join("G:/remote_apperance_gaze_dataset/processed_data/pre_trained_models_for_frame_method_eva/ui_dilated/user_" + str(user)+ "R"), 'w') as outfile:
