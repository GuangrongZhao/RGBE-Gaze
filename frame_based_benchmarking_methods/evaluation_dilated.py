...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_read_path",
                        ...
                        help="Path to read pre-trained models")
    parser.add_argument("--result_save_path",
                        default="G:/remote_apperance_gaze_dataset/processed_data/pre_trained_models_for_frame_method_eva/ui_dilated/",
    ...
    if args.network == "DilatedNet":
        ...
        state_dict = torch.load(
            os.path.join(args.model_read_path, 'dilated_L_eye.pt'),
    ...
    for user in userlist:
        ...
        for ii in range(user, user + 1):
            for jj in [1, 2, 3, 4, 5, 6]:
                ...
                f_label = h5py.File(os.path.join(
                    args.data_path, f"L_pitch_yaw_user_{ii}_exp{jj}.h5"),
                    'r')
        ...
        with open(os.path.join(args.result_save_path, f"user_{user}L"), 'w') as outfile:
