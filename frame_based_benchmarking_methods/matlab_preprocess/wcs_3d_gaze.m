function [Gazetimestamp,LGAZE,RGAZE]= combine_wcs_3d_gaze(data,R,t)
...
LPOGX_LIST = (data(:,10))*52.8;  %To multiply by screen width cm
LPOGY_LIST  = (data(:,11))*29.8; %Multiply by height cm
LPOGV_LIST = (data(:,12)); %To convert by screen coordinate system

LPOGZ_LIST  = zeros(length(LPOGY_LIST),1); %multiply the screen height cm
...
RPOGV_LIST = (data(:,15));
RPOGZ_LIST  = zeros(length(RPOGY_LIST),1);%To multiply the screen height cm
