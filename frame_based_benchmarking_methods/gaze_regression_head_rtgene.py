...
# from model import gaze360_model
...
if __name__ == "__main__":
    ...
    #################################### Configure data loader#################################################
        ...
        if args.network == "gaitcnn":
            ...
            model =  alexnet.model().to(device)
            optimizer = torch.optim.Adam(model.parameters())
        elif args.network == "resnet50":
            model =  resnet50.gaze_network().to(device)
            optimizer = torch.optim.Adam(model.parameters())
        elif args.network == "gaze360_model":
            model =  gaze360_model.GazeStatic().to(device)
            optimizer = torch.optim.Adam(model.parameters())
            loss_op = gaze360_model.PinBallLoss().cuda()
        elif args.network == "rtgene":
...
# 
        ...
        with open(os.path.join("/home/sduu2/userspace/zgr/remote_eyetracking/python_code/Tpami_final_result/", f"new_no_scale_RT-Gene_first"), 'w') as outfile:
    ...
    # [{i}/{length}
                ...
                if epoch % 5== 0:
                     torch.save(model.state_dict(), os.path.join('/home/sduu2/userspace/zgr/remote_eyetracking/python_code/Tpami_final_result/checkpoints/', f"new_Iter_{epoch}_no_scale_RT-Gene_first.pt"))
